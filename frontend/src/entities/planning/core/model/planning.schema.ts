...
export const LabMachineApiSchema = z.object({
    uuid: z.string().uuid(),
    salle_uuid: z.string().uuid(),
    name: z.string(),
    sort_order: z.number().int(),
});

export const LabMachineSchema = LabMachineApiSchema.transform((api) => ({
    uuid: api.uuid,
    salleUuid: api.salle_uuid,
    name: api.name,
    sortOrder: api.sort_order,
}));

export type LabMachine = z.infer<typeof LabMachineSchema>;

export const LabMachineCreateSchema = z.object({
    salleUuid: z.string().uuid(),
    name: z.string().min(1).max(100),
});

export type LabMachineCreate = z.infer<typeof LabMachineCreateSchema>;

export function labMachineCreateToApi(data: LabMachineCreate) {
    return {
        salle_uuid: data.salleUuid,
        name: data.name,
    };
}

export const LabMachineUpdateSchema = z.object({
    name: z.string().min(1).max(100).optional(),
    sortOrder: z.number().int().optional(),
});

export type LabMachineUpdate = z.infer<typeof LabMachineUpdateSchema>;

export function labMachineUpdateToApi(data: LabMachineUpdate) {
    return {
...
export const LabSalleApiSchema = z.object({
    uuid: z.string().uuid(),
    name: z.string(),
    sort_order: z.number().int(),
    machines: z.array(LabMachineApiSchema),
});

export const LabSalleSchema = LabSalleApiSchema.transform((api) => ({
    uuid: api.uuid,
    name: api.name,
    sortOrder: api.sort_order,
    machines: api.machines.map((m) => LabMachineSchema.parse(m)),
}));

export type LabSalle = z.infer<typeof LabSalleSchema>;

export const LabSalleListSchema = z.array(LabSalleSchema);

export const LabSalleCreateSchema = z.object({
    name: z.string().min(1).max(100),
});

export type LabSalleCreate = z.infer<typeof LabSalleCreateSchema>;

export function labSalleCreateToApi(data: LabSalleCreate) {
    return { name: data.name };
}

export const LabSalleUpdateSchema = z.object({
    name: z.string().min(1).max(100).optional(),
    sortOrder: z.number().int().optional(),
});

export type LabSalleUpdate = z.infer<typeof LabSalleUpdateSchema>;

export function labSalleUpdateToApi(data: LabSalleUpdate) {
    return {
        ...(data.name !== undefined && { name: data.name }),
        ...(data.sortOrder !== undefined && { sort_order: data.sortOrder }),
    };
